...
//        dump(check_ajax_referer(self::get_action_name()));
        ...
        if (isset($_POST[Options_Name::REDIRECT_URL])) {
            $data[Options_Name::REDIRECT_URL] = $_POST[Options_Name::REDIRECT_URL];
        }
        wp_send_json(
            $data
        );
    ...
    {
        echo sprintf('<input type="hidden" name="action" value="%1$s" />', self::get_action_name());
        ...
        wp_nonce_field(self::get_action_name(), "bc_form_security");
    }


    /**
     * @param $option_form_field
     * @param string $type
     * @param array|bool|float|int|mixed|string $default
     *
     * @return array|bool|float|int|mixed|string
     */
    private function get_option_value($option_form_field, $type = 'string', $default = '')
    {

        switch ($type) {
            //string is covered in default
            case 'int':
            ...
            case 'float':
                ...
                break;
            case 'bool':
                return $this->options->get_bool($option_form_field, $default);
                break;
            case 'array':
...
//        dump($current_value);
     ...
     * @return string
     */
    public function checkbox($setting_field_name, $disabled = false, $label = '', $default = false)
    {

        $current_value = $this->get_option_value($setting_field_name, 'bool', $default);
        $disabled = $disabled ? 'disabled' : '';
