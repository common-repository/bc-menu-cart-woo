...
(function (global, factory) {
    ...
    */
    ...
    function componentAPI(BCkit) {
        ...
        BCkit.component = function (name, options) {
            ...
            if (BCkit._initialized && !opt.functional) {
                ...
                fastdom.read(function () {
                    return BCkit[name](("[uk-" + id + "],[data-uk-" + id + "]"));
                });
            }
        ...
        BCkit.getComponents = function (element) {
            return element && element[DATA] || {};
        };
        BCkit.getComponent = function (element, name) {
            return BCkit.getComponents(element)[name];
        };
        ...
        return function (el, show) {
            ...
            return (show
                    ? Transition.start(el, assign({}, initProps, {
                        overflow: 'hidden',
                        height: endHeight
                    }), Math.round(duration * (1 - currentHeight / endHeight)), transition)
                    : Transition.start(el, hideProps, Math.round(duration * (currentHeight / endHeight)), transition).then(function () {
    ...
    function Core(BCkit) {
        ...
        ready(function () {
            ...
            on(window, 'load resize', function () {
                return BCkit.update(null, 'resize');
            });
        ...
        BCkit.icon.add = function (name, svg) {
            ...
            if (BCkit._initialized) {
                apply(document.body, function (el) {
                        return each(BCkit.getComponents(el), function (cmp) {
    ...
    function install$1(BCkit) {
        ...
        BCkit.modal.alert = function (message, options) {
            ...
            return new Promise(
                function (resolve) {
                    return on(BCkit.modal.dialog((" <div class=\"uk-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"uk-modal-footer uk-text-right\"> <button class=\"uk-button uk-button-primary uk-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'hide', resolve);
                }
            );
        };
...
(function ($) {
    ...
    $(document).on('mouseover', '.bc-mnc__cart-link--show-details-on-hover', function (e) {
        ...
        cart_details.addClass('cart-details-show');


    });

    /**
     * On mobile devices, if the user specify to move the menu to a specific position, do so
     */

    $(function () {
        var icons_to_move = $('[data-mobile-relative-element]');
        console.log('menu count: ', icons_to_move.length);
        _.each(icons_to_move, function (icon) {
            console.log('we are moving one icon');
            var position = $(icon).attr('data-mobile-position');
            console.log('element selector is: ', $(icon).attr('data-mobile-relative-element'));
            var to_element = $($(icon).attr('data-mobile-relative-element'));
            var remove = $(icon).attr('data-mobile-remove-origin') === '1';
            $(icon).removeAttr('data-mobile-position');
            $(icon).removeAttr('data-mobile-relative-element');
            $(icon).removeAttr('data-mobile-remove-origin');

            var clone = $(icon).clone();

            if (position === 'left') {
                clone.insertBefore(to_element);
            } else {
                clone.insertAfter(to_element);
            }
    ...
    $(document).on('click', '.bc-mnc__cart-link--show-details-on-click', function (e) {
        ...
        e.preventDefault();
        $(this).siblings('.bc-mnc__cart-details').addClass('cart-details-show');
    });

    //close button, general styles
    $(document).on('click', '.bc-mnc__cart-details--close-button', function (e) {
        $(this).closest('.bc-mnc__cart-details').removeClass('cart-details-show');
    });
